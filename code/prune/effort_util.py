...
class Effort_Trainer(Trainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        # self.TIME_BACKWARD += time.time()-start
        ...
        if self.get_batch_grad:
            ...
            for name, param in model.named_parameters():
                if ("base_model.model.model.layers.31.self_attn.v_proj.lora_B.default.weight" == name):
                    if param.grad is not None:
                        gradient = param.grad.reshape(-1)
                        print(f"使用参数梯度: {name}")
                        break
            if gradient is None:
                raise RuntimeError("未找到可用的LoRA参数梯度!")
            ...
            for s_id, s_loss in enumerate(loss):
                ...
                for name, param in model.named_parameters():
                    if ("base_model.model.model.layers.31.self_attn.v_proj.lora_B.default.weight" == name):
                        if param.grad is not None:
                            sample_grad = param.grad.reshape(1, -1)
                            break
                
                if sample_grad is None:
                    raise RuntimeError(f"未找到样本 {s_id} 的LoRA参数梯度!")
                gradient[s_id] = sample_grad
        # self.TIME_FETCH_GRAD += time.time()-start
        
        # self.TIME_ALL += time.time()-ssss
    ...
    def freeze_layers(self):
        ...
        for name, params in self.model.named_parameters():
            # 检查是否是我们要保留的LoRA参数 - 使用完整路径
            if ("base_model.model.model.layers.31.self_attn.v_proj.lora_B.default.weight" == name) and params.requires_grad:
                print(f"保留参数: {name}")
                continue
        ...
        for name, params in self.model.named_parameters():
            ...
            if ("base_model.model.model.layers.31.self_attn.v_proj.lora_B.default.weight" == name):
                try:
                    ...
                    print(f"无法启用参数 {name}: {e}")
            # 启用第31层MLP参数（如果需要）
            elif ("base_model.model.model.layers.31.mlp" in name and "lora" in name):
                try:
                    params.requires_grad = True
                    found_params.append(name)
                    print(f"启用MLP参数: {name}")
                except Exception as e:
                    print(f"无法启用MLP参数 {name}: {e}")
        
        if not found_params:
            print("❌ 警告：没有找到任何可启用的LoRA参数!")
            # 打印所有包含lora的参数供调试
            print("所有LoRA参数:")
            for name, params in self.model.named_parameters():
                if "lora" in name.lower():
                    print(f"  {name} (requires_grad={params.requires_grad})")
        else:
            print(f"✅ 成功启用 {len(found_params)} 个参数")
...
# for redefining forward function of LlamaforCausalLM
...
class Modified_LlamaModel(LlamaModel):
    ...
    def forward(
        ...
        # print("** after all layers. hidden state", hidden_states)
...
class Modified_LlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(
        ...
        # print("** hidden state shape of the last layer:", hidden_states.shape)
